...
test("validate succeeds and fails", (t) => {
  ...
  t.deepEqual(
    validate(constraints, {accept: true, name: "Sam", password: "abc123"}),
    {valid: true, errors: {}},
  );
  t.deepEqual(
    validate(constraints, {accept: false, name: null, password: "abc"}),
    {
      valid: false,
      errors: {
        accept: ["required"],
...
test("custom validator", (t) => {
  const constraints = {
    ...
    confirm_password: {
      custom: (value: any, values: {[key: string]: any}): string[] => {
        return values.password !== value ? ["must match password"] : [];
      },
    },
  };
  t.deepEqual(
    validate(constraints, {password: "abc123", confirm_password: "abc123"}),
    {valid: true, errors: {}},
  );
  t.deepEqual(
    validate(constraints, {password: "abc123", confirm_password: "invalid"}),
    {
      valid: false,
      errors: {
